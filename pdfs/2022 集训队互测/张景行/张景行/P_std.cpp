#include"bigint.cpp"
#include<bits/stdc++.h>
using namespace std;
//dengyaotriangle!

bigint n,phi;
int ps;
vector<bigint> res;

bigint qpw(bigint x,int pw){
    bigint res=1;
    while(pw){
        if(pw&1)res=res*x;
        x=x*x;pw>>=1;
    }
    return res;
}


bigint qpwm(bigint x,bigint pw,bigint n){
    bigint res=1;
    int r=pw.digit();
    for(int i=0;i<r;i++){
        if((pw.data[i>>6]>>(i&63))&1)res=res*x%n;
        x=x*x%n;
    }
    return res;
}
mt19937_64 rng(chrono::steady_clock().now().time_since_epoch().count());
bigint rand(bigint thres){
    bigint ret;
    do{
        ret.data.resize(thres.data.size());
        for(int i=0;i<(int)ret.data.size();i++){
            ret.data[i]=rng();
        }
        int h=thres.digit()-(thres.data.size()-1)*64;
        if(h!=64)ret.data.back()&=((unsigned long long)1<<h)-1;
        while(!ret.data.empty()&&!ret.data.back())ret.data.pop_back();
    }while(ret>=thres||ret==0);
    return ret;
}
bigint gcd(bigint a,bigint b){
    int sft=0;
    while(a!=0){
        if(a>b)swap(a,b);
        else{
            bool fa=a.data[0]&1,fb=b.data[0]&1;
            if(fa&&fb)b-=a;
            else if(fa)b>>=1;
            else if(fb)a>>=1;
            else{
                a>>=1;b>>=1;sft++;
            }
        }
    }
    return b<<sft;
}
void factor(bigint n){
    if(n==1)return;
    while(~n.data[0]&1){
        res.push_back(2);n>>=1;
    }
    const int thres=25;
    int sd=2*(n.digit()+1);
    bigint s=(bigint(1)<<sd)/n;
    for(int i=0;i<thres;i++){
        bigint x=rand(n);
        bigint g0=gcd(x,n);
        if(g0!=1){ 
            factor(g0);factor(n/g0);
            return;
        }
        bigint a=1;
        int r=phi.digit();
        for(int i=0;i<r;i++){
            if((phi.data[i>>6]>>(i&63))&1){
                a=a*x;a-=((a*s)>>sd)*n;if(a>=n)a-=n;
            }
            x=x*x;x-=((x*s)>>sd)*n;if(x>=n)x-=n;
        }
        for(int j=0;j<ps;j++){
            bigint b=a*a;b-=((b*s)>>sd)*n;if(b>=n)b-=n;
            if(b==1){
                bigint g=gcd(a-1,n);
                if(g!=1&&g!=n){
                    factor(g);
                    factor(n/g);
                    return;
                }
                break;
            }
            a=b;
        }
    }
    for(int i=n.digit()-1;i>=2;i--){
        bigint x;
        for(int j=n.digit()/i;j>=0;j--){
            if(qpw(x+(bigint(1)<<j),i)<=n)x+=bigint(1)<<j;
        }
        if(qpw(x,i)==n){
            for(int j=0;j<i;j++)res.push_back(x);
            return;
        }
    }
    res.push_back(n);
}
const int thres=1e6;

bool ntp[thres];
int prm[thres],prc;
int main(){
    cin>>n>>phi;
    cerr<<n.digit()<<'\n';
    while(~phi.data[0]&1)phi>>=1,ps++;
    for(int i=2;i<thres;i++){
        if(!ntp[i]){
            prm[++prc]=i;
            while(1){
                int rem=0;
                for(int j=n.data.size()-1;j>=0;j--){
                    rem=((long long)(rem)<<32)%i;
                    rem=((long long)(rem)<<32)%i;
                    rem=(rem+n.data[j]%i)%i;
                }
                if(rem)break;
                res.push_back(i);
                n/=i;
            }
        }
        for(int j=1;i*prm[j]<thres;j++){
            ntp[i*prm[j]]=1;
            if(i%prm[j]==0)break;
        }
    }
    factor(n);
    sort(res.begin(),res.end());
    cout<<res.size()<<'\n';
    for(auto &x:res)cout<<x<<'\n';
    return 0;
}